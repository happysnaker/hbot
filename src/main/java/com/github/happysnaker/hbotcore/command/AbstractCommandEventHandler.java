package com.github.happysnaker.hbotcore.command;


import com.github.happysnaker.hbotcore.exception.CanNotParseCommandException;
import com.github.happysnaker.hbotcore.exception.InsufficientPermissionsException;
import com.github.happysnaker.hbotcore.handler.GroupMessageEventHandler;
...
public abstract class AbstractCommandEventHandler extends GroupMessageEventHandler implements CommandHandler, CommandHandlerManager {
    ...
    public List<MessageChain> handleMessageEvent(GroupMessageEvent event, Context ctx) {
        ...
        try {
            ...
            fail(event, e);
            return List.of(buildMessageChain(getQuoteReply(event), "命令解析出错，说明：" + e.getMessage()));
        } catch (InsufficientPermissionsException e) {
            fail(event, "权限不足：" + e.getMessage());
            return List.of(buildMessageChain(getQuoteReply(event), "对不起，您没有足够的权限，说明：" + e.getMessage()));
        } catch (Exception e) {
            fail(event, e);
            return List.of(buildMessageChain(getQuoteReply(event), "异常错误，说明：" + e.getMessage()));
        }
        if (ans != null) {
            success(event);
        }
